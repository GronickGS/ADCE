...
class AplicacionGestorcomponentes:
    def __init__(self, root):
        ...
        self.imagen_usuario = tk.PhotoImage(file="004.png").subsample(5, 5)  # tamaño de imagen
        ...
        self.boton_agregar = tk.Button(self.marco_agregar, text="Agregar", font=("Courier New", 10, "bold"), command=self.serieagregar_componente, bg="red", fg="white", borderwidth=2, relief="raised")
        ...
        self.arbol = ttk.Treeview(self.marco_mostrar, columns=('Nombre', 'serie'), show='headings')
    ...
    def crear_tabla(self):
        ...
        cursor.execute('''CREATE TABLE IF NOT EXISTS componentes
                          (id INTEGER PRIMARY KEY, nombre TEXT, Serie TEXT)''')
        self.conexion.commit()

    def serieagregar_componente(self):
        nombre = self.entrada_nombre.get().capitalize()  # Capitaliza la primera letra del nombre
        Serie = self.entrada_Serie.get()

        if nombre and Serie:
            cursor = self.conexion.cursor()
            cursor.execute('INSERT INTO componentes (nombre, Serie) VALUES (?, ?)', (nombre, Serie))
            self.conexion.commit()
    ...
    def mostrar_componentes(self, componentes=None):
        ...
        for componente in componentes:
            self.arbol.insert('', 'end', values=(componente[1], componente[2]))

    def exportar_a_csv(self):
        if self.componentes:
            nombre_archivo = "componentes.csv"
            with open(nombre_archivo, 'w', newline='') as csvfile:
                nombres_columnas = ['Nombre', 'serie']
                escritor_csv = csv.DictWriter(csvfile, fieldnames=nombres_columnas)
                escritor_csv.writeheader()
                for componente in self.componentes:
                    escritor_csv.writerow({'Nombre': componente[1], 'serie': componente[2]})
    ...
    def editar_componente(self):
        ...
        if elemento_seleccionado:
            ...
            boton_guardar = tk.Button(ventana_editar, text="Guardar Cambios", command=lambda: self.guardar_cambios(elemento_seleccionado, entrada_nombre.get(), entrada_Serie.get(), ventana_editar), bg="red", fg="white")
    ...
    def guardar_cambios(self, elemento_seleccionado, nuevo_nombre, nuevo_Serie, ventana_editar):
        cursor = self.conexion.cursor()
        cursor.execute('UPDATE componentes SET nombre=?, Serie=? WHERE nombre=? AND Serie=?',
                       (nuevo_nombre, nuevo_Serie, self.arbol.item(elemento_seleccionado, 'values')[0], self.arbol.item(elemento_seleccionado, 'values')[1]))
        self.conexion.commit()
